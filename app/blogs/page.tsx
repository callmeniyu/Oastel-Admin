...
export default function BlogsPage() {
  ...
  const blogsTableData = blogs.map((blog) => ({
    ...
    featured: (
      <select
        defaultValue={blog.featuredRank || 0}
        onChange={async (e) => {
          const rank = parseInt(e.target.value, 10);
          try {
            // Use NEXT_PUBLIC_API_URL to ensure we call the backend server (not the Next frontend route)
            const apiBase = process.env.NEXT_PUBLIC_API_URL || "";
            const res = await fetch(
              `${apiBase}/api/blogs/${blog._id}/feature`,
              {
                method: "PATCH",
            ...
            toast.success("Featured blogs updated");
            fetchBlogs();
          } catch (err: any) {
            console.error("Failed to update featured rank", err);
            toast.error(err.message || "Failed to update featured rank");
          }
        }}
